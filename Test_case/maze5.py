...
class MazeGame:
    ...
    def draw_maze(self):
        for row in range(self.grid_size):
            for col in range(self.grid_size):
                ...
                if self.maze[row][col] == 1:
                    if "wall" not in self.images:
                        self.images["wall"] = tk.PhotoImage(file="Image\Maze\level\wallStone.png")  # Replace with actual image path
                    self.canvas.create_image(x1 + self.cell_size // 2, y1 + self.cell_size // 2, image=self.images["wall"])
                else:
                    self.canvas.create_rectangle(x1, y1, x2, y2, fill="white", outline="gray")

        # Draw player
        if "player" not in self.images:
            self.images["player"] = tk.PhotoImage(file="Image\Maze\hero\hitA\hero_hitA_0000.png")  # Replace with actual image path
        ...
        # Draw exit
        if "exit" not in self.images:
            self.images["exit"] = tk.PhotoImage(file="Image\Maze\level\groundExit.png")  # Replace with actual image path
