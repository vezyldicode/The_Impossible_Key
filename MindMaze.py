...
class MazeGame:
    def __init__(self, root):
        ...
        thread.start()
    def UI_setup(self):
        setting_Image = Image.open("assets/ico/nav.png")
        self.setting_button = ctk.CTkButton(self.nav_bar,
                                text = "",
                                image= ctk.CTkImage(setting_Image),
    ...
    def setting_tab(self):
        ...
        """Tạo frame cho chỉnh seed"""
        #label cho seed
        self.seed_frame = ctk.CTkFrame(self.settings_frame, fg_color=self.bg)
        self.seed_frame.pack(padx =5, pady =5, fill='x')
        self.seed_label = ctk.CTkLabel(self.seed_frame,
                                        text=("Seed:"),
                                        font=self.textfont, 
                                        text_color=self.textcolor)
        ...
        #phần hiển thị và chỉnh sửa seed
        self.seed_entry = ctk.CTkEntry(self.seed_frame,
                                       width=200,
                                       border_width=0,
                                       font=self.textfont,
                                       text_color=self.textcolor,
        ...
        #label chú thích cho seed
        self.seed_help = ctk.CTkLabel(self.seed_frame,
                                        text = "Click on seed to copy the seed code to clipboard. Type 'none' or 'random' to use a random seed.",
                                        wraplength= 700,
                                        font=(self.textfont_family, 20),
                                        text_color=self.textcolor)
        self.seed_help.pack(side="left", anchor = "s")
        
        """Tạo frame cho chỉnh overlay"""
        self.overlay_change_frame = ctk.CTkFrame(self.settings_frame, fg_color=self.bg)
        self.overlay_change_frame.pack(padx =5, pady =10, fill='x')
        self.shadow_switch = ctk.CTkCheckBox(self.overlay_change_frame,
                                             text = "Shadow", font=self.textfont, 
                                             text_color=self.textcolor,
                                             command=lambda: setattr(self, "isOverlay", not self.isOverlay))
        self.shadow_switch.select()
        self.shadow_switch.pack(side ="top", padx = 20, anchor = "w")
    ...
    def close_settings(self):
        def comfirm_prompt():
            ...
            confirm_popup.title("Mind Maze")
            confirm_popup.geometry("300x150")
            popup_label = ctk.CTkLabel(confirm_popup,
                                    text="Change Seed?",
                                    wraplength= 200,
                                    font=self.textfont)
            ...
            def confirm():
                ...
                self.seed_input= self.seed_entry_value
            ...
            replay_button = ctk.CTkButton(confirm_popup, text="Yes", command=confirm)
            quit_button = ctk.CTkButton(confirm_popup, text="No", command=cancel)
            replay_button.pack(side=tk.LEFT, padx=20, pady=10)
            quit_button.pack(side=tk.RIGHT, padx=20, pady=10)
        
        if self.seed_entry.get() == "none" or self.seed_entry.get() == "random":
            self.seed_input = None
            self.settings_frame.pack_forget()
            self.main_frame.pack(fill="both", expand=True)
            self.reset_game()
        try:
            self.seed_entry_value= int(self.seed_entry.get())
        except:
            self.settings_frame.pack_forget()
            self.main_frame.pack(fill="both", expand=True)
            return
        if self.seed_entry_value!=self.maze_seed:
            comfirm_prompt()
        else:
            self.settings_frame.pack_forget()
            self.main_frame.pack(fill="both", expand=True)
    ...
    def check_password(self, event):
        ...
        # else:
        #     return True
        return True if self.is_allDone else False
    ...
    def win_game(self):
        def replay_prompt():
            ...
            popup.title("Mind Maze")
            popup.geometry("300x150")
            popup_label = ctk.CTkLabel(popup,
            ...
            replay_button = ctk.CTkButton(popup, text="Replay", command=replay)
            quit_button = ctk.CTkButton(popup, text="Quit", command=quit_game)
            replay_button.pack(side=tk.LEFT, padx=20, pady=10)
            quit_button.pack(side=tk.RIGHT, padx=20, pady=10)
